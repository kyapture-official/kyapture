...
export default function SubscriptionGrid() {
  ...
  const galleryPercent = calculatePercent(usage.galleriesUsed, limits.max_galleries)
  const storagePercent = calculatePercent(usage.storageUsedGb, limits.storage_gb)
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <section className="bg-white p-5 rounded-2xl border border-cream-200 space-y-3 shadow-sm">
          <div className="flex justify-between items-baseline">
            <span className="text-xs text-muted font-bold uppercase tracking-wider">Collections</span>
            <span className="text-sm font-bold text-ink">
              {usage.galleriesUsed} <span className="text-xs text-muted font-light">/ {limits.max_galleries}</span>
            </span>
          </div>
        ...
        <section className="bg-white p-5 rounded-2xl border border-cream-200 space-y-3 shadow-sm">
          <div className="flex justify-between items-baseline">
            <span className="text-xs text-muted font-bold uppercase tracking-wider">Photos Uploaded</span>
            <span className="text-sm font-bold text-ink">{usage.photosUsed}</span>
          </div>
          <p className="text-[10px] text-muted font-light leading-relaxed">
            Across all your collections. Each gallery can hold up to{' '}
            <strong className="text-ink font-semibold">{limits.max_photos_per_gallery}</strong> photos.
          </p>
        </section>
        ...
        <section className="bg-white p-5 rounded-2xl border border-cream-200 space-y-3 shadow-sm">
          <div className="flex justify-between items-baseline">
            <span className="text-xs text-muted font-bold uppercase tracking-wider">Disk Storage</span>
            <span className="text-sm font-bold text-ink">
              {parseFloat(usage.storageUsedGb).toFixed(2)} <span className="text-xs text-muted font-light">/ {limits.storage_gb} GB</span>
            </span>
          </div>
          <div className="h-2 w-full bg-cream-100 rounded-full overflow-hidden">
            <div
